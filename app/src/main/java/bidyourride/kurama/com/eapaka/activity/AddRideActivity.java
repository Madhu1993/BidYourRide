...
import bidyourride.kurama.com.eapaka.model.Category;
import bidyourride.kurama.com.eapaka.model.DirectionObject;
import bidyourride.kurama.com.eapaka.model.GsonRequest;
import bidyourride.kurama.com.eapaka.model.Overview_polyline;
import bidyourride.kurama.com.eapaka.model.RideRequest;
import bidyourride.kurama.com.eapaka.model.RouteObject;
import bidyourride.kurama.com.eapaka.model.User;
import bidyourride.kurama.com.eapaka.rest.Helper;
import bidyourride.kurama.com.eapaka.rest.VolleySingleton;
import bidyourride.kurama.com.eapaka.view.CircleImageView;

import static bidyourride.kurama.com.eapaka.activity.RideDetailsActivity.REP_DELAY;

/**
 * Created by madhukurapati on 3/7/18.
 */

public class AddRideActivity extends BaseActivity implements AdapterView.OnItemSelectedListener, View.OnFocusChangeListener, TimePickerDialog.OnTimeSetListener, DatePickerDialog.OnDateSetListener, View.OnClickListener, View.OnTouchListener {
    ...
    private ImageView decPersonCount, incPersonCount;
    ...
    private TextView personCount, errorMaxPerson;
    private Switch cashSwitch;
    private LinearLayout personCountLL;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cashSwitch = findViewById(R.id.cash_switch);
        ...
        personCountLL = findViewById(R.id.person_count_ll);
        incPersonCount = findViewById(R.id.person_inc_button);
        personCount = findViewById(R.id.person_count);
        errorMaxPerson = findViewById(R.id.error_max_person_count);
        errorMaxPerson.setVisibility(View.GONE);

        decPersonCount = findViewById(R.id.person_dec_button);
        incPersonCount.setOnClickListener(this);
        decPersonCount.setOnClickListener(this);
        incPersonCount.setOnTouchListener(this);
        decPersonCount.setOnTouchListener(this);

        if (mCount == 1) {
            decPersonCount.setVisibility(View.GONE);
        }

        submit.setOnClickListener(this);
        submit.setTextColor(getResources().getColor(R.color.button_unclickable, null));
        cashSwitch.setChecked(false);
        cashSwitch.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (cashSwitch.isChecked()) {
                    submit.setClickable(true);
                    submit.setTextColor(getResources().getColor(R.color.grey_1, null));
                } else {
        ...
        incPersonCount.setOnLongClickListener(
        ...
        decPersonCount.setOnLongClickListener(
                new View.OnLongClickListener() {
                    public boolean onLongClick(View arg0) {
                        mAutoDecrement = true;
                        repeatUpdateHandler.post(new RptUpdater());
    ...
    private void setPersonCountLayout() {
        if (typeOfRequest.equals("1")){
            if(personCountLL.getVisibility() == View.GONE){
                personCountLL.setVisibility(View.VISIBLE);
            }
            return;
        }

        if (typeOfRequest.equals("2")){
            if(personCountLL.getVisibility() == View.VISIBLE){
                personCountLL.setVisibility(View.GONE);
            }
        }
    ...
    private void setMaxPersonErrorView() {
        errorMaxPerson.setVisibility(View.VISIBLE);
        errorMaxPerson.setText(getResources().getText(R.string.max_person_error));
        errorMaxPerson.setTextColor(Color.RED);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.person_inc_button:
                increment();
                return;
            case R.id.person_dec_button:
                decrement();
                return;
    ...
    public void decrement() {
        if (mCount == 1) {
            ...
            if (mCount == 1) {
                ...
                if (errorMaxPerson.getVisibility() == View.VISIBLE) {
                    errorMaxPerson.setVisibility(View.GONE);
                }
            }
            personCount.setText(String.valueOf(mCount));
        }
    }
    ...
    public void increment() {
        ...
        if (decPersonCount.getVisibility() != View.VISIBLE) {
            decPersonCount.setVisibility(View.VISIBLE);
        }
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            if (user == null) {
                                ...
                                try {
                                    ...
                                    writeNewRide(userId, user.username, capTitle, originFullAddress, destinationFullAddress, typeOfRequest, date, time, originCityName, destinationCityName, originLatString, originLongString, destinationLatString, destinationLongString, directionObjectJson, encodedMapString, mCount);
                                } catch (Exception e) {
                                    writeNewRide(userId, user.username, title, originFullAddress, destinationFullAddress, typeOfRequest, date, time, originCityName, destinationCityName, originLatString, originLongString, destinationLatString, destinationLongString, directionObjectJson, encodedMapString, mCount);
                                }
                            }
                            finish();
                        }
    ...
    private void writeNewRide(String userId, String username, String capTitle, String origin, String destination, String typeOfRequest, String date, String time, String originCityName, String destinationCityName, String originLat, String originLong, String destinationLat, String destinationLong, String directionObjectJson, String encodedMapString, int personCount) {
        String key = mDatabase.child("rides").push().getKey();

        RideRequest rideRequest = new RideRequest(userId, username, capTitle, origin, destination, typeOfRequest, date, time, originCityName, destinationCityName, originLat, originLong, destinationLat, destinationLong, directionObjectJson, encodedMapString, personCount);
