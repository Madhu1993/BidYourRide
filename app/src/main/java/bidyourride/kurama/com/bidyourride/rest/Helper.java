...
public class Helper {
    ...
    public static String getStaticImageApi(double originLat, double originLong, double destinationLat, double destinationLong, String polyEncode) {
        String s = Helper.STATIC_IMAGE_API + "&path=weight:10|enc:" + polyEncode + "&maptype=roadmap&weight:3|color:black|geodesic:true|" + /*"&markers=color:red|label:O|" + originLat + "," + originLong +*/ "&&markers=color:red|label:D|" + destinationLat + "," + destinationLong + "|" + "&key=" + API_KEY;
        return s;
